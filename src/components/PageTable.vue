<template>
  <a-collapse :bordered="false" class="collapse-total" expandIconPosition="right">
    ...
    <a-col :xs="18" :sm="12" :md="12" :lg="6">
      <a-input-number v-model:value="displayDto" @change="calcula"  addon-before="DTO" :min="0" :max="99">%</a-input-number>
    </a-col>
    ...
    <a-row>
      ...
      <a-row>
        <a-col :span="12">
          &nbsp<a-checkbox :checked="isIva" @click="checkIva">+21%IVA</a-checkbox></a-col>
          <a-col :span="12" class="text-right">
          {{iva.toFixed(2)}}
        </a-col>
      </a-row>
      <a-row>
        <a-col :span="12">
          &nbsp<a-checkbox :checked="isRe" @click="checkRe">+5.2%R.E.</a-checkbox></a-col>
          <a-col :span="12" class="text-right">
          {{re.toFixed(2)}}
        </a-col>
      </a-row>
  ...
  <a-row>
    <a-col :span="24">
            <a-button @click="addProducto" class="btn-add" type="text" block><template #icon><PlusOutlined /></template></a-button>
      <a-list class="a-list" item-layout="horizontal" :data-source="dataSource" :locale="{ emptyText: ' ' }">
  ...
  <div class="delete-button">
    <a-button size="large" type="text" danger @click="deleteProducto(item.key, item.euros)">x</a-button>
  </div>
</div>
          </a-list-item>
        </template>
 ...
 <a-row>
  ...
  <a-modal
    ...
    <a-button @click="goCompany">下一步</a-button>
</template>
...
import { initAllTable,  
        insertArticulo, queryAllTree, queryAllArticulo, selectClient} from '../util/dbSqlite'
